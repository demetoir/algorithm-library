...
procedure qsort (var p:partinal; l, r:integer);
var i, j:integer;
    x, y:partinal;
begin
  if l>=r then exit;
  with p^ do begin
    i:=l; j:=r; x:=d[(l+r) shr 1];
    repeat
      while compare (d[i], x)<0 do inc (i);
      while compare (x, d[j])<0 do dec (j);
      if i<=j then begin
        y:=d[i]; d[i]:=d[j]; d[j]:=y;
        inc (i); dec (j);
      end;
...
var i:integer;
begin
  if compare (p, numbers[length (p.d)])=0 then write (length (p.d))
  else
  with p^ do
