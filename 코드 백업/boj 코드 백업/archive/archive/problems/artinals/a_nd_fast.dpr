...
var d, e : integer;
begin
  Result := 0;  if X = Y then exit;
  d := 0;  e := 0;
  repeat
    if X = Y then break;
    if X.s = 0 then begin d := -1; break end;
    if Y.s = 0 then begin d := 1;  break end;
    if (X.v >= 0) and (Y.v >= 0) then begin d := X.v - Y.v;  break end;
    if (X.s = 1) and (Y.s = 1) then begin X := X.A;  Y := Y.A;  continue end;
    if X.s = 1 then begin e := -1;  Y := Y.A;  continue end;
    if Y.s = 1 then begin e := 1;   X := X.A;  continue end;
    if X.A.s >= Y.s then begin e := 1;   X := X.A;  continue end;
    if Y.A.s >= X.s then begin e := -1;  Y := Y.A;  continue end;
    assert (X.A.s = Y.A.s);
    if X.A = Y.A then begin X := X.B;  Y := Y.B end
    		 else begin X := X.A;  Y := Y.A;  e := 2 end
  until false;
  if d = 0 then begin d := e;  assert (e <> 2) end;
  if d > 0 then Result := 1 else if d < 0 then Result := -1 else assert(false)
end;
