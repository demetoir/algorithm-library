...
program macro;
const
  MaxN = {1000}512;  MaxL = 254;  MaxV = 1000;  MaxObj=100000;
...
begin
  hs := ((cardinal(A) mod HPrime) + (cardinal(B) mod HPrime)*17
        + cardinal(s)*239) mod HPrime;
  H := OH[hs];
...
function objind (Z : PNode) : indtype;
var i : integer;  X, Y : PNode;
begin
  Result := 0;  i := MaxDepth;  X := Z;
  repeat
    Y := X.U;  if Y = nil then begin Z.fo := Result;  exit end;
    if X = Y.L then Result := Result * 0.5 - 1 else
    if X = Y.R then Result := Result * 0.5 + 1 else runerror (239);
    X := Y;  dec (i)
  until i = 0;
  runerror (239)
end;
...
var t : indtype;
begin
  if X = Y then Result := 0 else begin
    t := objind(X) - objind(Y);
    if t < 0 then Result := -1 else if t > 0 then Result := 1
    else begin runerror (239); Result := 0 end;
  end
end;
...
function list2node (const X : array of PNode) : PNode;
begin Result := list2nodei (X, low(X), high(X)) end;
...
begin
  ...
  FO[0] := Empty;  Empty.v := 0;
  HV[hashstr('0')] := Empty;
  for N := 1 to high(FO) do begin
    FO[N] := list2nodei (FO, 0, N-1);
    FO[N].v := N;
    if N <= MaxN then HV[hashstr(inttostr(N))] := FO[N];
    TotV := 0
  end
end;
...
var A, B, C  : nodearr;  i, j, L : integer;  v1, v2 : indtype;
begin
  ...
  if Z.v = 0 then begin F0 := (Y.v <> 0);  Result := Y;  exit end;
  if Y.v = 0 then begin 
    F0 := false;
  ...
  if length(B) = 0 then v1 := infty else v1 := objind (B[0]);
  if length(C) = 0 then v2 := infty else v2 := objind (C[0]);
  while (i < length(B)) or (j < length(C)) do begin
    ...
    if F1 then begin
      inc(i);  if i < length(B) then v1 := objind(B[i]) else v1 := infty 
    end;
    if F2 then begin
      inc(j);  if j < length(C) then v2 := objind(C[j]) else v2 := infty 
    end;
  end;
  Result := list2nodei (A, 0, L-1);
  setlength (B, 0);  setlength (C, 0);
  setlength (A, 0)
end;
...
var h : indtype;  t : PNode;  i, j : integer;
begin
  if u >= v then exit;
  h := A[(u + v) shr 1].fo;  i := u;  j := v;
  repeat
    while A[i].fo < h do inc (i);
    while A[j].fo > h do dec (j);
...
begin
  ...
  if CL <> '}' then expect ('}');
  getlexem;
  for i := 0 to L-1 do objind (A[i]);
  qsort (A, 0, L-1);  j := 1;
  for i := 1 to L - 1 do 
    if A[i] <> A[i-1] then begin A[j] := A[i];  inc (j) end;
  Result := list2nodei (A, 0, j-1);
  setlength (A, 0)
end;
